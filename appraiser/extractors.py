...
def embeddings_vec(filename, model, class_idx, feature_extractor=extractor):
    ...
    sv_full = feature_extractor(images=image, return_tensors="pt").to("cuda")[
        "pixel_values"
    ]
    vec_for_search = model.get_input_embeddings()(sv_full.cpu()).squeeze()[
        class_idx
    ]
